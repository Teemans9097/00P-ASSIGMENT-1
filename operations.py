class LibraryManagementSystem:
    ...
    def add_book(self, isbn, title, author, genre, total_copies):
        ...
        if genre not in self.valid_genres:
            print(f"Error: Invalid genre! Must be one of: {self.valid_genres}")
            return None

        # Validate total copies
        if total_copies <= 0:
            return False, "Error: Total copies cannot be less or equal to zero!"
    ...
    def add_member(self, name, email):
        ...
        member_id = f"T{self.next_member_id:03d}"
        self.next_member_id += 1
